...
mod sevenseg; //driver module for sevenseg

/// Support routines for debugging I/O.
pub mod io;

mod hello; //use the driver module for hello
...
const FAULT_RESPONSE: kernel::procs::FaultResponse = kernel::procs::FaultResponse::Panic; //the good one
...
struct STM32F412GDiscovery {
    ...
    //sevenseg driver
    sevenseg: &'static sevenseg::SevenSeg<stm32f412g::gpio::Pin<'static>>,
}
...
pub unsafe fn reset_handler() {
    ...
    let sevenseg = static_init!(
        sevenseg::SevenSeg<stm32f412g::gpio::Pin<'static>>,
        sevenseg::SevenSeg::new(
            PinId::PG12.get_pin().as_ref().unwrap(),
            PinId::PG11.get_pin().as_ref().unwrap(),
            PinId::PG10.get_pin().as_ref().unwrap(),
        )
    );
